	...
	def GetWinsInfos(self):
		...
		for win_hwnd in lst_enum_window_hwnd:
			...
			if buff.value == s_npp_class:
				...
				self.GetWindowThreadProcessId(win_hwnd, ctypes.pointer(ci_win_pid))
		...
		# get all class="Scintilla" windows handle, childs of NPP window, in lst_scint_hwnd
...
class C_Block_UnBlock_Input():
	...
	def GetWinsHandle(self):
		...
		self.npp_win_hwnd, self.lst_win_hwnd, s_npp_class, s_scint_class = o_get_nppscintilla_wins.GetWinsInfos()
