...
# an already existing active selection(last in multi-selection) can also be expanded by executing one of those selection gestures inside it
# if option 'get_brackets' or 'get_quotes' is disabled a second selection inside the selection will however select them

# (i) brackets/quotes matching : this script will match brackets/quotes even in different folding level/folded lines/hidden lines,
	# but both brackets must be of the same style, both quotes AND their content must have the same style
...
i_option_control_right_click				= 1 # select word : alphanumeric a-z/A-Z/0-9 with '_' and custom special characters
...
s_option_word_special_chars = r"@.-" # add custom special characters for word selection (in addition to alphanumeric a-z/A-Z/0-9 with '_')
...
s_option_word_special_chars = "".join(set(s_option_word_special_chars)) # remove duplicates in word special chars
...
t_option_verbdes = (
	...
	"CTRL + right-click"				+ "\t\t\t\t"	+ ": select the whole word : alphanumeric a-z/A-Z/0-9 with '_'" + s_opt_info_wordchr, \
