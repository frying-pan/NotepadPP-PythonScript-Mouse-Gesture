...
class C_Find_OutRange():
	...
	def FindText_MatchingQuotes_OutRange(self, curedit, i_start, i_end):
		...
		# matching quotes can not cross other styles
		for t_findtext_prev, t_findtext_next in \
				((t_findtext_prev_single, t_findtext_next_single), (t_findtext_prev_double, t_findtext_next_double)):
			if not(t_findtext_next is None):
				i_style = curedit.getStyleAt(t_findtext_next[0])
				lst_style = curedit.getStyledText(t_findtext_prev[0], t_findtext_next[0])[1]
				for i in range(0, len(lst_style)):
					if lst_style[i] != i_style:
						if t_findtext_next == t_findtext_next_single:	t_findtext_next_single = None
						else:											t_findtext_next_double = None
						break
		...
		while True:
			...
			if (i_pos_open != self.INVALID_POSITION and i_pos_open < i_start):
				...
				if (b_angle and b_sl_angle):
					if curedit.getTextRange(i_pos_open, i_pos_open + 1) == "<":
